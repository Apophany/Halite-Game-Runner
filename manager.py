...
halite_command = "E:\Git\Halite\game_files\halite.exe"
replay_dir = "replays/replay_hlts"
...
class Database:
    ...
    def recreate(self):
        ...
        try:
            cursor.execute(
                "create table games(id integer primary key, game_id integer, name text, finish integer, field_size integer, map_size integer, map_seed integer, timestamp date, replay_file text)")
            cursor.execute(
                "create table players(id integer primary key, name text unique, path text, lastseen date, rank integer default 1000, skill real default 0.0, mu real default 25.0, sigma real default 8.33,ngames integer default 0, active integer default 1)")
            self.db.commit()
        except:
            pass
    ...
    def add_player(self, name, path):
        self.update("insert into players values(?,?,?,?,?,?,?,?,?,?)",
                    (None, name, path, self.now(), 1000, 0.0, 25.0, 25.0 / 3.0, 0, True))
    ...
    def update_player_skill(self, name, skill, mu, sigma):
        self.update("update players set ngames=ngames+1,lastseen=?,skill=?,mu=?,sigma=? where name=?",
    ...
    def reset_ranks(self):
        self.update("update players set rank=?, skill=?, mu=?, sigma=?, ngames=?", (1000, 0.0, 25.0, 25.0 / 3.0, 0))
        pass
...
class Player:
    ...
    def __repr__(self):
        return "%s\t%s\t%d\t%3f\t%3f\t%3f\t%d\t%d" % (
        self.name, self.last_seen, self.rank, self.skill, self.mu, self.sigma, self.ngames, self.active)
...
class Commandline:
    def __init__(self):
        ...
        self.parser.add_argument("-R", "--reset-ranks", dest="resetRanks",
                                 action="store_true", default=False,
                                 help="Reset ranks for bots in the database")
    ...
    def act(self):
        ...
        elif self.cmds.resetRanks:
            print("Resetting ranks for all bots")
            self.manager.db.reset_ranks()
